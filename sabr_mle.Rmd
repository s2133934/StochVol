...
### Colins email - page 87 of the YUIMA DOCS 
...
A different approach to putting in a 2D model into YUIMA. ITs in the poisson.random.sampling method section, but its of the SABR type.
...
cor.mod <- setModel(drift=c("",""), diffusion=diff.coef.matrix, solve.variable=c("x1", "x2"), xinit=c(3,2))
set.seed(111)
...
library(yuima)
## Set a model
diff.coef.1 <- function(x1, x2, beta=1) { 
  ret <- (x2 * x1^(beta)) 
  return(ret) 
}
diff.coef.2 <- function(x1, x2, alpha=1) alpha*x2
cor.rho <- 0.7 
diff.coef.matrix <- matrix(c("diff.coef.1(x1, x2, beta)", "diff.coef.2(x1, x2, alpha)*cor.rho", "", "diff.coef.2(x1, x2, alpha)*sqrt(1-cor.rho^2)"),2,2)
cor.model <- setModel(drift=c("",""), diffusion=diff.coef.matrix, solve.variable=c("x1", "x2"), xinit=c(0.1,0.1))
set.seed(111)
...
mle <- qmle(yuima, start = param.init, lower = low.par, upper = upp.par) 
summary(mle)
```
...
sabr.mod <- setModel(drift = c("0", "mu*g(t)*x3", "mu*x3"), diffusion = diff.coef.matrix, state.variable = c("x1", "x2", "x3"), solve.variable = c("x1", "x2", "x3"))
