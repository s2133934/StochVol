...
set.seed(12345, kind="L'Ecuyer-CMRG") #I dont know what this method is ?! 
a0 <- 0.01 #1
b0 <- -0.1 #2
s0 <- 0.1 #0.5
g0 <- 0 #0.3
...
sim <- snssde1d(drift=f,diffusion=g,x0=2,N=10^3,Dt=10^-3)
mydata <- sim$X

fx <- expression( theta[1]+theta[2]*x ) ## drift coefficient of model
gx <- expression( theta[3]*x^0 ) ## diffusion coefficient of model 

fitmod <- fitsde(data = mydata, drift = fx, diffusion = gx, start = list(theta1=1, theta2=1, theta3=1),pmle="euler")
fitmod
```
```{r}
coef(fitmod)
...
logLik(fitmod)
```
Not too shabby as I got 4300 in Excel, but why are the parmeters so bad?! 
...
```
Again, the theta3, which is my sigma, is fine. The theta2 isnt so bad here - its comparing -0.1 to 0.7 which although bad isnt as bad as theta1, which is wildly out. ANY IDEAS?! 
